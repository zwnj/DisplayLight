...
DisplayLightは、次の三種類の電源操作を一つの通知領域アプリから扱います。

- **ディスプレイ消灯設定**：利用者の明示操作により、現在のWindows電源プランのAC時またはバッテリー時の値を変更します。
- **スリープ防止要求**：利用者が手動で有効にする一時的な要求で、Windows電源プラン自体は変更しません。
- **手動ディスプレイ消灯**：利用者が3秒のカウントダウンを完了した場合だけ、接続中のディスプレイへ一回限りの消灯要求を送ります。

この三種類は、変更の持続期間と復旧方法が異なるため、同じ抽象化にまとめません。
...
外側クリックでは、クリック先のウィンドウがアクティブになった後も閉じる面を最前面に保ち、約170ミリ秒のモーション完了後に非表示にします。
クリック先が一瞬だけ手前へ描画されることを避けるため、最前面状態はフライアウトの表示開始から非表示完了まで保持し、非表示後に解除します。
通知アイコンとフライアウトは、下と上のタスクバーでは水平中心、左右のタスクバーでは垂直中心を原則として揃えます。
中央揃えで画面外へ出る場合は、作業領域から12 DIP相当の余白を残す位置へ補正します。

表示時はトップレベルウィンドウを最終位置へ固定し、フライアウト面をタスクバー側へ隠した位置から約250ミリ秒のcubic ease-outで移動します。
終了時は約170ミリ秒のcubic ease-inで同じ面をタスクバー側へ戻し、完了後にウィンドウを非表示にします。
背景、外周、影、内容を含む`FlyoutSurface`を一つのVisualとして移動し、トップレベルウィンドウの位置はフレームごとに変更しません。
Visualの変換はWPFの描画フレームへ同期し、最終位置へ到達した後に内容だけを約200ミリ秒でフェードインします。
Desktop Acrylicは固定されたウィンドウ全体へ描かれるため、Visualの移動中だけ無効化し、完成位置へ背景だけが残る状態を防ぎます。
展開、折りたたみ、エラー表示によって必要な高さが変わる場合は、通知アイコン側の辺を基準に位置とサイズを約180ミリ秒で同時に補間します。
...
開く途中の再クリックは閉じる操作へ切り替えます。
非アクティブ化による閉じ処理が始まった後に同じ通知アイコンの選択イベントが届く場合があるため、閉じる途中の選択イベントは再表示として扱いません。
...
「ディスプレイをオフ」は消灯時間カードの下へ補助操作として配置します。
押すと同じボタン内で`3秒後にオフ`、`2秒後にオフ`、`1秒後にオフ`と表示し、再押下、Escape、外側クリック、フライアウトを閉じる操作でキャンセルします。
ゼロへ到達した場合はフライアウトを閉じるモーションの完了後に一度だけ消灯要求を送ります。

色は意味別の動的リソースへ集約し、Windowsのライトテーマ、ダークテーマ、高コントラストへ追従します。
Windows 11の背景効果を外観候補として検証し、利用できない場合と高コントラストでは単色サーフェスとDWM角丸で操作性を維持します。
最外周の角丸とWPF前景サーフェスをWindows 11のフライアウト標準である8 DIPへ揃えます。
DWMの非クライアント枠は無効化し、開閉時にVisualと一緒に移動するWPF前景サーフェスだけで外周線を描きます。
...
9. 手動ディスプレイ消灯は、3秒のカウントダウンと利用者が到達可能なキャンセル手段を省略しません。
...
手動ディスプレイ消灯には、`WM_SYSCOMMAND`の`SC_MONITORPOWER`へ値`2`を渡すWindowsの定義済みシステムコマンドを使用します。
ブロードキャストによる他プロセスの待機を避け、DisplayLight自身のウィンドウハンドルを`DefWindowProcW`へ渡します。

この操作は電源プラン、スリープ防止要求、保存設定を変更しません。
マウスやキーボードなど、Windowsが認識する入力による再点灯を妨げません。
カウントダウン状態はプロセス内だけに保持し、フライアウトを閉じた場合やアプリ終了時に破棄します。
自動テストではネイティブ呼び出しをテストダブルへ置き換え、実際のディスプレイは消灯しません。
...
1. WPFを開始する前にVelopackの更新ライフサイクルを処理します。
2. インストールまたは更新の完了時は、現在ユーザーのスタートアップ登録を不足時だけ作成します。
3. 単一インスタンス用の所有権を取得します。
4. OSアダプター、設定ストア、更新サービス、ViewModelを生成します。
5. 非表示のWPFフライアウトからウィンドウハンドルを作成します。
6. 通知領域アイコンを追加します。
7. 通常起動では通知アイコンの近くへフライアウトを表示し、スタートアップ起動では非表示を維持します。
8. 設定と現在のWindows電源状態を読み込みます。
9. 起動から20秒後に、インストール版だけ安定版の更新を確認します。
10. 終了要求では電源要求を解放し、通知領域アイコンを削除します。
11. 終了処理の一部が失敗しても、残りの解放処理を継続します。

二重起動したプロセスは、既存プロセスへ表示要求を送り、自身は電源状態を変更せず終了します。
ただし、スタートアップによる二重起動は既存フライアウトを表示しません。
...
UIは成功通知を表示せず、変更後の読戻しが完了した場合だけ現在値を更新します。

- OS操作失敗：実行しようとした操作の近くへWindowsエラーコードを表示します。
- 全体操作失敗：再読み込みなど対象を一つに限定できないエラーだけをヘッダー付近へ表示します。
...
インストール版は、起動後と利用者の明示操作時にGitHub Releasesへ更新を問い合わせます。
更新通信で送るのはGitHub Releasesの取得に必要な通常のHTTP情報だけであり、設定、電源状態、作業ディレクトリ、ログを送信しません。
公開リポジトリへの問い合わせは認証トークンを使用しません。

Agent Previewで名前付きパイプを採用する場合は現在ユーザーへ限定し、受信サイズ、JSONの深さ、文字列長を制限します。
作業ディレクトリを扱う場合はUI表示に必要な間だけプロセス内で保持し、設定と通常ログへ保存しません。

ログの既定レベル、保存期間、利用者による消去方法はQ16で決定します。
...
配布物はVelopackで生成する自己完結型のSetupとPortable ZIPです。
Setupで導入したインストール版だけが、GitHub Releasesの安定版を更新元としてアプリ内更新を利用できます。
Portable ZIPはインストールせず試す用途とし、アプリ内更新を適用しません。

起動から20秒後の確認は更新が見つかった場合だけ画面へ通知します。
手動確認では、最新版、インストール版ではない状態、確認失敗も画面へ表示します。
ダウンロードと再起動は利用者が「更新して再起動」を押した場合だけ行います。
更新適用前に手動消灯のカウントダウンとスリープ防止要求を解除します。

インストール版は、`HKCU\Software\Microsoft\Windows\CurrentVersion\Run`へVelopackランチャーと`--startup`を登録します。
この登録はWindowsのタスクマネージャーに表示され、利用者が「スタートアップ アプリ」から有効または無効を切り替えます。
DisplayLightは既存の登録内容が正しい場合に値を書き直さず、Windowsが保持する無効状態を変更しません。
Portable版は登録せず、アンインストール時は登録値を削除します。
自動起動の判断は`docs/adr/0007-user-startup-registration.md`に記録します。

コード署名は未決です。
判断の詳細は`docs/adr/0006-velopack-updates.md`に記録します。
