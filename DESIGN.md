...
表示時はタスクバー側の32 DIP離れた位置から220ミリ秒で移動し、終了時はタスクバー側へ16 DIPを140ミリ秒で移動します。
表示開始時の不透明度は72%とし、モーションの前半で100%へ到達させます。
移動と同時に不透明度を切り替え、`SystemParameters.ClientAreaAnimation`が無効な場合は即時に開閉します。
