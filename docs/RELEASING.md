...
pwsh -NoProfile -File ./scripts/package.ps1 -Version 0.1.3 -NoRestore
```
...
git switch main
git pull --ff-only
git tag -a v0.1.3 -m "DisplayLight 0.1.3"
...
- インストールまたは更新後にタスクマネージャーの「スタートアップ アプリ」へ登録される。
- スタートアップ起動でフライアウトが表示されない。
- アンインストール後にスタートアップ登録が残らない。
